...
namespace Student
{
    ...
    public class UDPer_client_Kau
    {
        ...
        // KAU
        public int packentNum = MainPage.TOTAL_PACKETS; // 전체 패킷 수 (필요에 맞게 수정)
        ...
        private static int messageLength = MainPage.MESSAGE_LENGTH;

        // MainPage의 LogMessage 호출을 위한 참조
        private readonly Action<string> _logMessage;

        public UDPer_client_Kau(Action<string> logMessage)
        {
            try
            {
        ...
        public void Start() {
            ...
            try {
                ...
                Debug.WriteLine("buffer size" + udp.Client.ReceiveBufferSize);
        ...
        public void Stop() {
            try
            {
                if (udp != null)
                {
                    Debug.WriteLine("UDP Stopped");
                    udp.Close();
                    udp = null;
                }
            }
            catch (Exception ex) { 
                Debug.WriteLine($"Error stopping UDP client: {ex.Message}");
            }
        ...
        private void StartListening() {
            Debug.WriteLine("UDP Listening Started");
            // ?. 연산자 udp가 null이 아닐때만
        ...
        private void Receive(IAsyncResult ar) {
            try {
                ...
                if (teacherIPAddress == null)
                {
                    ...
                    Debug.WriteLine($"Teacher ID: {teacherIPAddress}");
                    ConnectToServer();
                }
                else
                {
                    if (OnReceiveMessage != null)
                    {
                            ...
                            (localIPAddress != null)
                        {
                            ...
                            if (receivedMessageNum == message_num)
                            {
                                ...
                                // Debug.WriteLine($"Received {message_num} message");
                            }
                            else
                            {
                                Debug.WriteLine("Received wrong message");
                            }
                        }
        ...
        public static void SetNewMsgBit(int packetNum)
        {
            ...
            if ((checkMessageChunk[byteIndex] & (1 << bitIndex)) == 0)
            {
                ...
                // Debug.WriteLine($"Set checkMessageChunk[{packetNum}]:");
            ...
            try
            {
                ...
                if (!checkMessageChunk.SequenceEqual(lastMessageChunk))
                {
                    ...
                    if (AllBitsOne(checkMessageChunk))
                    {
                        ...
                        Debug.WriteLine("All packets received.");
                        ...
                        // 다음 메시지 준비
                        receivedMessageNum++;
                        Debug.WriteLine($"receivedMessageNum: {receivedMessageNum}");
                    }
                ...
                Debug.WriteLine($"Error UDPMessage Check: {e.Message}");
            }
         }
        ...
        */
        ...
        public void SendTCPMessage(string message) {
            try
            {
                if (client != null && stream != null && client.Connected)
                {
                    ...
                    Debug.WriteLine($"TCP message sent to server .");
                }
                else
                {
                    Debug.WriteLine("Client is not connected to server. Attempting to reconnect...");
                    Reconnect();
                }
            }
            catch (Exception ex)
            {
                Debug.WriteLine($"Error sending TCP message: {ex.Message}");
                Reconnect();
            }
        }
        ...
        private void Reconnect() {
            ...
            for (int i = 0; i < retryCount; i++)
            {
                try
                {
                    Debug.WriteLine($"Reconnecting attempt {i + 1}/{retryCount}...");
                    ConnectToServer();
                    if (client != null && client.Connected)
                    {
                        Debug.WriteLine("Reconnected to server");
                        return;
                    }
                }
                catch (Exception ex)
                {
                    Debug.WriteLine($"Reconnection attempt {i + 1} failed: {ex.Message}");
                }
